...
ns.view = (function() {
    ...
    return {
        ...
        build_table: function(person) {
            ...
            // did we get a person array?
            if (person) {
                for (let i=0, l=person.length; i < l; i++) {
                    rows += `<tr><td class="fname">${person[i].fname}</td><td class="lname">${person[i].lname}</td><td>${person[i].timestamp}</td></tr>`;
                }
